...
export function createLocationService({geo,secure=true,onChange=()=>{}}={}) {
  ...
  return {
    ...
    request(){
      ...
      if(!secure||!geo)return publish({status:'unavailable',coordinates:null,message:'Location is unavailable here. Open JavaMile in your browser over HTTPS, or retry after enabling location.'});
      publish({status:'loading',coordinates:null,message:'Finding your location… Allow location access if your browser asks.'});
      const failure=error=>{if(requestId!==generation)return;publish({status:error.code===1?'denied':'unavailable',coordinates:null,message:error.code===1?'Location access is blocked. Allow location for this site in your browser settings, then retry. No sample cafés will be substituted.':error.code===3?'Location took too long. Try again, or retry after enabling location.':'Your device couldn’t find its location. Check location services, then retry or check browser location settings.'})};
